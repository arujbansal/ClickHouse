-- Non-identifier expressions in WITH clause must have an alias.
-- Bare identifiers are allowed for backward compatibility with older ClickHouse versions
-- that produce identifier-only WITH elements when rewriting queries in mixed-version clusters.

-- 1 element, no alias (literals and expressions must have aliases)
